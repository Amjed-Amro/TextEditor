...
import com.amjed.TextEditor.services.internal.CaesarCipher;
    ...
    public String encrypt(int key, String input) {
        ...
        for (Integer i = 0; i < sb.length(); i++) {
            ...
            if (ch.isUpperCase(ch)) {
                ...
                sb.setCharAt(i, shiftedAlphabet.charAt(alphaIndex));
            }
            if (ch.isLowerCase(ch)) {
                Integer alphaIndex = Constants.ALPHABET.LOWER.indexOf(ch);
                sb.setCharAt(i, lowerShiftedAlphabet.charAt(alphaIndex));
            }
        }
        return sb.toString();
    }
        ...
        for (int i = 0; i < sb.length(); i++) {
            ...
            if (i % 2 == 0) {
                if (ch.isUpperCase(ch)) {
                    ...
                    sb.setCharAt(i, shiftedAlphabet1.charAt(alphaIndex));
                }
                if (ch.isLowerCase(ch)) {
                    int alphaIndex = Constants.ALPHABET.LOWER.indexOf(ch);
                    sb.setCharAt(i, lowerShiftedAlphabet1.charAt(alphaIndex));
                }
            } else {
                if (ch.isUpperCase(ch)) {
                    int alphaIndex = Constants.ALPHABET.UPPER.indexOf(ch);
                    sb.setCharAt(i, shiftedAlphabet2.charAt(alphaIndex));
                }
                if (ch.isLowerCase(ch)) {
                    int alphaIndex = Constants.ALPHABET.LOWER.indexOf(ch);
                    sb.setCharAt(i, lowerShiftedAlphabet2.charAt(alphaIndex));
                }
            }
        }
        return sb.toString();
